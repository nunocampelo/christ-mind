...
| 20260923T182131Z | v3.1 | dev | 0.49 / 0.55 | 0.47 / 0.53 | 0.64 / 0.72 | v3.1: added explicit `expresses` and `contrasts_with` rules (aligning the prompt to gold conventions already set in step 5; no relabelling). Mechanism: t1-1-3, t1-1-21 flipped `is`→`expresses` and t1-1-12 flipped `is`→`contrasts_with`, matching gold. |
| 20260923T210128Z | v3.1 | dev | 0.53 / 0.58 | 0.52 / 0.57 | 0.67 / 0.74 | v3.1 confirm run 2 of N=3 |
| 20260923T210317Z | v3.1 | dev | 0.49 / 0.53 | 0.47 / 0.51 | 0.66 / 0.71 | v3.1 confirm run 3 of N=3 |

**v3.1, N=3 dev (mean ± stdev):** loose 0.503±0.021 / 0.554±0.024 · strict 0.485±0.027 / 0.534±0.030 · relaxed 0.657±0.017 / 0.723±0.012 (60±1 matched). The two targeted patterns held in all three runs. Relaxed R 0.699→0.723 (+0.024, ~2σ; the v3 and v3.1 relaxed-R distributions barely overlap) is a real gain. Strict R moved too (0.498→0.534) but with larger stdev (0.030), so it's softer. Still below the 0.8 holdout gate; the remaining gap is dominated by surface-modifier subject/object differences and gold-vs-model convention edges — the entity-resolution layer's job, not more prompt iteration.
